// ── Config ──────────────────────────────────────────────────────────────────
let deviceId     = "device_1"; // Device ID assigned in the Serial Bridge app
const MAX_HIST   = 120;

// ── State ────────────────────────────────────────────────────────────────────
let idInput, connectButton, downloadButton;
let bridge;
let serialBuffer = []; // Accumulates lines between separator blocks
let sensorData   = null;
let historyData  = [];
...
function setup() {
  ...
  let lbl = createSpan('Device ID');
  lbl.parent(bar);
  lbl.style('color', '#6e78a0');
  lbl.style('font-size', '12px');
  lbl.style('font-family', 'system-ui, sans-serif');

  idInput = createInput(deviceId);
  idInput.parent(bar);
  idInput.attribute('placeholder', 'device_1');
  idInput.style('width', '120px');
  idInput.style('padding', '5px 10px');
  idInput.style('background', '#151820');
  idInput.style('color', '#dde2f5');
  idInput.style('border', '1px solid #2a2f44');
  idInput.style('border-radius', '5px');
  idInput.style('font-family', '"SF Mono", Consolas, monospace');
  idInput.style('font-size', '12px');
  ...
  // Connect to Serial Bridge (app must be running on localhost:3000)
  bridge = new SerialBridge();

  bridge.onData('*', (data, id) => {
    if (id !== deviceId) return;
    console.log(data);
    handleSerialLine(data.trim());
  });

  bridge.onStatus('*', (status, port, id) => {
    if (id !== deviceId) return;
    connStatus = (status === 'open') ? 'connected' : 'error';
  });
}

function doDownload() {
  if (historyData.length === 0) return;
  let rows = ['time,pm2p5,co2,temp,humidity,voc,nox,pm1'];
  for (let d of historyData) {
    rows.push([d.time, d.pm2p5, d.co2, d.temp, d.humidity, d.voc, d.nox, d.pm1].join(','));
  }
  let blob = new Blob([rows.join('\n')], { type: 'text/csv' });
  let a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'ambient-data.csv';
  a.click();
...
// Called for each line received from the serial device via SerialBridge.
// Lines are accumulated until a separator (–––) arrives, then parsed as a block.
function handleSerialLine(line) {
  if (!line || /^[^\w\d.]+$/.test(line)) {  // empty or only non-alphanumeric (separator)
    if (serialBuffer.length > 0) {
      let res = parseSerialBlock(serialBuffer);
...
function parseSerialBlock(lines) {
  let res = {};
  for (let line of lines) {
    let m;
    if ((m = line.match(/^PM1\.0:\s*([\d.]+)/i)))  res.pm1p0       = parseFloat(m[1]);
    if ((m = line.match(/^PM2\.5:\s*([\d.]+)/i)))  res.pm2p5       = parseFloat(m[1]);
    if ((m = line.match(/^C[oO]2:\s*([\d.]+)/i)))  res.co2         = parseFloat(m[1]);
    if ((m = line.match(/^VOC:\s*([\d.]+)/i)))     res.vocIndex    = parseFloat(m[1]);
    if ((m = line.match(/^NO[Xx]:\s*([\d.]+)/i)))  res.noxIndex    = parseFloat(m[1]);
    if ((m = line.match(/^Temp:\s*([\d.]+)/i)))    res.temperature = parseFloat(m[1]);
    if ((m = line.match(/^RH:\s*([\d.]+)/i)))      res.humidity    = parseFloat(m[1]);
  }
  return res;
}
...
function drawGraph() {
  ...
  textStyle(NORMAL);
  textAlign(LEFT, TOP);
  text("HISTORY — ALL SENSORS", x + 14, y + 13);

  const activeData = historyData;
  const activeN    = MAX_HIST - 1;
  ...
  fill(...C.muted);
  textSize(9);
  const maxTicks   = floor(gw / 80);           // ~80 px minimum gap
